...
|PIR 감지 (SSR 유지)|	감지(1)|	ON	     |  -	            | SSR 유지	|이미 SSR이 ON 상태 유지|
...
# 작업 진행 부분 
### 송신부와 수신부를 나눠 송신부는 센서와 SSR의 작동 상태를 판단해 에러 상황 발생 시 수신부에 정보 전송 역할

### 송신부에서 발송한 정보를 가상 RTU를 사용해 수신부에서 시뮬레이션 (USART3) 

### [송신 MCU] → USART3 (TX) → E22-900T22S → [LoRa 무선 통신] → E22-900T22S → USART3 (RX) → [수신 MCU] → 가상 RTU 데이터 처리

# 문제 
### 송신부에서 잘 제어해서 다른 센서나 SSR 작동은 이상이 없지만
### 임의로 문제 상황을 발생시켜보고 수신부에서 데이터 수신이 안되는 상황 
# 의심 부분
### 동일 채널 , 동일 통신 속도인지 확인 필요
